...
(async () => {
  ...
  await page.goto('https://www2.commsec.com.au/quotes/derivatives?stockCode=BHP&exchangeCode=ASX', {waitUntil: 'networkidle2'});
  ...
  const derivativesLink = "body > app-root > div > div > sub-nav > nav > ul > li:nth-child(10) > a";
  await page.waitForSelector(derivativesLink);
  await page.click(derivativesLink);

  // click on the strike filters
  const strikes = "body > app-root > div > div > div > div.subnav-content__inner.section.section--width-max > derivatives-container > div > div > div > options > div > div.subnav-content-container__head.ng-star-inserted > div.subnav-content-container__filters.ng-star-inserted > filter-drop-down:nth-child(2) > div > div > div.filter-drop-down__selected-wrapper > div.filter-drop-down__button-wrapper > button";
 ...
 const expiry = "body > app-root > div > div > div > div.subnav-content__inner.section.section--width-max > derivatives-container > div > div > div > options > div > div.subnav-content-container__head.ng-star-inserted > div.subnav-content-container__filters.ng-star-inserted > filter-drop-down-multi-level > div > div > div.filter-drop-down__selected-wrapper > div.filter-drop-down__button-wrapper > button";
 await page.waitForSelector(expiry);
 await page.click(expiry);

 await wait(1000);
 await page.click('xpath/'+'/html/body/app-root/div/div/div/div[1]/derivatives-container/div/div/div/options/div/div[1]/div[2]/filter-drop-down-multi-level/div/div/div[2]/div/div[1]/ul/li[2]');


  await wait(1000);
  const tableSelector = 'body > app-root > div > div > div > div.subnav-content__inner.section.section--width-max > derivatives-container > div > div > div > options > div > div.subnav-content-container__body > table > tbody';
  ...
  for (let i = 1; i < tableData.length + 1; i++) {
    ...
    if (currentRow !== undefined && currentRow.length > 0 ){
      ...
      if (!isNaN(parseInt(checkCallSales))) {
        if (+currentRow[5] <= 2.00 || +currentRow[12] === 0) {
          keepTrackCalls.push(currentRow[0]);
        }
      }
      if (!isNaN(parseInt(checkPutSales))) {
        if (+currentRow[12] <= 2.00 || +currentRow[12] === 0) {
          keepTrackPuts.push(currentRow[0]);
        }
      }
    }
  }
