...
(async () => {
  ...
  await wait(1000);
  await page.goto('https://www2.commsec.com.au/quotes/derivatives?stockCode=BHP&exchangeCode=ASX', {
    waitUntil: 'networkidle2',
  });
  ...
  async function checkMultipleExpiry(number: number) {
    ...
    for (let i = 1; i < tableData.length + 1; i++) {
      ...
      if (currentRow !== undefined && currentRow.length > 0) {
        ...
        if (!isNaN(parseInt(checkCallSales))) {
          if (+currentRow[5] <= 0.5 || +currentRow[5] === 0) {
            keepTrackCalls.push(currentRow[0]);
          }
        }
        if (!isNaN(parseInt(checkPutSales))) {
          if (+currentRow[12] <= 0.5 || +currentRow[12] === 0) {
  ...
  await checkMultipleExpiry(2);
  await checkMultipleExpiry(3);
  await checkMultipleExpiry(4);
  await checkMultipleExpiry(5);
